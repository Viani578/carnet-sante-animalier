...
def generate_health_book_pdf(data, carnet_id):
    ...
    # ===== PAGE 4 : ATTESTATION VÉTÉRINAIRE =====
    
    # Titre de l'attestation
    story.append(Paragraph(
        "🩺 ATTESTATION VÉTÉRINAIRE OFFICIELLE",
        ParagraphStyle(
    ...
    <b>Je soussigné(e), {data.get('attestation_veterinaire', '[Nom du vétérinaire]')}</b><br/>
    Vétérinaire diplômé(e) - N° Ordre: {data.get('attestation_ordre', '[Numéro d\'ordre]')}<br/><br/>
    
    <b>Atteste avoir examiné l'animal :</b><br/>
    <b>Nom :</b> {data.get('name', '')}<br/>
    <b>Espèce :</b> {data.get('species', '')}<br/>
    <b>Race :</b> {data.get('breed', '')}<br/>
    <b>Âge :</b> {data.get('age', '')}<br/>
    <b>N° Identification :</b> {data.get('identification', 'Non spécifié')}<br/><br/>
    
    <b>Date de l'examen :</b> {data.get('attestation_date', '')}<br/>
    <b>Validité de l'attestation :</b> {data.get('attestation_validite', 'Non spécifiée')}<br/><br/>
    
    <b>Observations médicales :</b><br/>
    {data.get('attestation_observations', 'Aucune observation particulière. L\'animal est en bonne santé.')}
    """
    
    attestation_frame = Table([[Paragraph(attestation_content, 
                                       ParagraphStyle(name='AttestationText', fontSize=12))]], 
                            colWidths=[16*cm])
    attestation_frame.setStyle(TableStyle([
        ('BOX', (0,0), (0,0), 2, COLOR_SECONDARY),
    ...
    story.append(attestation_frame)
    story.append(Spacer(1, 3*cm))
    ...
    if data.get('cachet') and os.path.exists(data['cachet']):
        try:
            img_cachet = Image(data['cachet'], width=5*cm, height=5*cm)
            img_cachet.hAlign = 'LEFT'
            signature_data.append(img_cachet)
        except:
            pass
    ...
    signature_text = f"""
    <div align="center">
    <b>Signature du vétérinaire</b><br/>
    Fait le {data.get('attestation_date', datetime.now().strftime('%d/%m/%Y'))}<br/>
    {data.get('attestation_veterinaire', '')}
    </div>
    """
    
    signature_para = Paragraph(signature_text, 
                             ParagraphStyle(name='SignatureText', fontSize=12, alignment=1))
    signature_data.append(Spacer(1, 1*cm))
    signature_data.append(signature_para)
    
    # Signature si disponible
    if data.get('signature') and os.path.exists(data['signature']):
        try:
            img_signature = Image(data['signature'], width=7*cm, height=3*cm)
            img_signature.hAlign = 'CENTER'
            signature_data.append(img_signature)
        except:
            pass
    
    # Table pour aligner cachet et signature
    sig_table = Table([[signature_data]], colWidths=[16*cm])
